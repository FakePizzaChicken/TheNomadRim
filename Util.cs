...
namespace TheNomadRim
{
    public class Global
    {
        public static List<LightsaberBlade> g_all_blades = new List<LightsaberBlade>();
    }
    ...
    public class Util
    {
        ...
        public static void PlaySoundLooped(AudioSource source, AudioContainer audioContainer = null, float volume = 0)
        {
            if (source)
            {
                if (audioContainer != null) source.clip = audioContainer.PickAudioClip(UnityEngine.Random.Range(0, audioContainer.sounds.Count - 1));
                source.volume = volume == 0 ? source.volume : volume;
                source.loop = true;
                source.Play();
            }
            return;
        ...
        public static void PlaySound(AudioSource source, AudioContainer audioContainer = null, float volume = 0)
        {
            if (source)
            {
                source.volume = volume == 0 ? source.volume : volume; source.loop = false;
        ...
        public static void CleanCustomDataProperly<T>(Item item) where T : ContentCustomData
        {
            if (item.HasCustomData<T>())
            {
                DebugService.LogInfo($"Custom Data of type {typeof(T).Name} found");

                item.RemoveCustomData<T>();

                if (item.TryGetCustomData(out T data))
                {
